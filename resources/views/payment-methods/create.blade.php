<x-app-layout>
    <x-slot name="header">
        <h1 class="font-bold text-slate-800 text-xl">Add Payment Method</h1>
        <p class="text-sm text-slate-500 mt-1">Create a new payment source or account</p>
    </x-slot>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden w-full relative">
        ...
        <div class="p-5 sm:p-10">
            <form method="POST" action="{{ route('payment-methods.store') }}" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus
                                   placeholder="e.g. Bank BCA or Cash"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" id="balance" name="balance" value="{{ old('balance', 0) }}"
                                   placeholder="0.00"
                                   class="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-sm">
                        </div>
