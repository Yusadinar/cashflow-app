<x-app-layout>
    <x-slot name="header">
        <h1 class="font-bold text-slate-800 text-base truncate">Categories</h1>
        <p class="text-[10px] sm:text-xs text-slate-400 truncate">Manage transaction categories</p>
    </x-slot>
    ...
    <div class="flex flex-col md:grid md:grid-cols-2 gap-20 md:gap-8 lg:gap-12 pb-16">
        @foreach(['income' => 'Income Categories', 'expense' => 'Expense Categories'] as $type => $title)
            @php
                $filteredCategories = $categories->where('type', $type);
            @endphp
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr class="border-b border-slate-100 bg-slate-50/50">
                                    ...
                                    <th class="py-3.5 px-6 font-semibold text-xs text-slate-500 uppercase tracking-wider text-center w-28">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50">
                                @foreach($filteredCategories as $category)
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        <td class="py-3 px-6 text-sm font-medium text-slate-700">{{ $category->name }}</td>
                                        <td class="py-3 px-6">
                                                            ...
                                                            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:flex sm:items-center sm:p-0">
                                                                ...
                                                                <div x-show="showModal"
                                                                    ...
                                                                    <div class="flex items-start gap-4 mb-5">
                                                                        ...
                                                                        <div>
                                                                            <h3 class="text-lg font-bold text-slate-800" id="modal-title">Delete Category</h3>
                                                                            <p class="mt-2 text-sm text-slate-500 leading-relaxed">Are you sure you want to delete <span class="font-bold text-slate-700">"{{ $category->name }}"</span>? This action cannot be undone and may fail if transactions are still linked to it.</p>
                                                                        </div>
                                                                    </div>
                                ...
                                @if($filteredCategories->isEmpty())
                                    <tr>
                                        <td colspan="2" class="py-12 text-center text-slate-400 text-sm">No {{ strtolower(explode(' ', $title)[0]) }} categories found.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
