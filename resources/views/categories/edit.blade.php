<x-app-layout>
    <x-slot name="header">
        <h1 class="font-bold text-slate-800 text-xl">Edit Category</h1>
        <p class="text-sm text-slate-500 mt-1">Update transaction category</p>
    </x-slot>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select id="type" name="type" required
                                    class="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-sm">
                                <option value="expense" {{ old('type', $category->type) == 'expense' ? 'selected' : '' }}>Expense (Outcome)</option>
                                <option value="income" {{ old('type', $category->type) == 'income' ? 'selected' : '' }}>Income (Income)</option>
                            </select>
                        </div>
