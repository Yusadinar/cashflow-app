<x-app-layout>
    <x-slot name="header">
        <h1 class="font-bold text-slate-800 text-xl">Add Transfer</h1>
        <p class="text-sm text-slate-500 mt-1">Move funds between payment methods</p>
    </x-slot>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden w-full relative">
        ...
        <div class="p-5 sm:p-10">
            <form method="POST" action="{{ route('transfers.store') }}" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8">
                    ...
                    <div class="space-y-8">
                        <div>
                            <label for="transfer_date" class="block text-sm font-semibold text-slate-700 mb-2">Date</label>
                        ...
                        <div>
                            <label for="from_payment_method_id" class="block text-sm font-semibold text-slate-700 mb-2">From Account</label>
                            <select id="from_payment_method_id" name="from_payment_method_id" required
                                    class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-sm">
                                <option value="" disabled {{ old('from_payment_method_id') ? '' : 'selected' }}>Select origin...</option>
                                @foreach($paymentMethods as $pm)
                                    <option value="{{ $pm->id }}" {{ old('from_payment_method_id') == $pm->id ? 'selected' : '' }}>
                                        {{ $pm->name }}
                                    </option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->get('from_payment_method_id')" class="mt-2" />
                        </div>

                        <div>
                            <label for="to_payment_method_id" class="block text-sm font-semibold text-slate-700 mb-2">To Account</label>
                            <select id="to_payment_method_id" name="to_payment_method_id" required
                                    class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-sm">
                                <option value="" disabled {{ old('to_payment_method_id') ? '' : 'selected' }}>Select destination...</option>
                                @foreach($paymentMethods as $pm)
                                    <option value="{{ $pm->id }}" {{ old('to_payment_method_id') == $pm->id ? 'selected' : '' }}>
                                        {{ $pm->name }}
                                    </option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->get('to_payment_method_id')" class="mt-2" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" placeholder="Optional details..."
                                      class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-sm">{{ old('description') }}</textarea>
