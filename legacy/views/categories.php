...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        $result = $catCtrl->store($userId, $_POST['name'] ?? '', $_POST['type'] ?? '');
        $msg    = $result['success'] ? $result['message'] : '';
        $error  = $result['success'] ? '' : implode(', ', $result['errors'] ?? [$result['message']]);

    } elseif ($action === 'update') {
        $result = $catCtrl->update($userId, (int)$_POST['id'], $_POST['name'] ?? '', $_POST['type'] ?? '');
        $msg    = $result['success'] ? $result['message'] : '';
        $error  = $result['success'] ? '' : implode(', ', $result['errors'] ?? [$result['message']]);

    } elseif ($action === 'delete') {
        $result = $catCtrl->destroy($userId, (int)$_POST['id']);
        $msg    = $result['success'] ? $result['message'] : '';
